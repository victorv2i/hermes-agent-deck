...
describe('Wordmark', () => {
  it('renders the "Hermes Agentdeck" accessible name in the real wordmark font', () => {
    render(<Wordmark />)
    const text = screen.getByText('Hermes Agentdeck')
  ...
  it('renders the brand mark as the raster "AD" wing image (fixed identity), not an inline SVG', () => {
    ...
    // Decorative; the visible "Hermes Agentdeck" text carries the accessible name.
  ...
  it('exposes the "Hermes Agentdeck" wordmark as the lockup accessible name', () => {
    render(<Wordmark />)
    expect(screen.getByText('Hermes Agentdeck')).toBeInTheDocument()
  })
